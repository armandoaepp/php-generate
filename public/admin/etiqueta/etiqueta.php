...
<body>
      ...
      <nav class="full-content" aria-label="breadcrumb">
        <ol class="breadcrumb breadcrumb-shape shadow-sm radius-0">
          ...
          <li class="breadcrumb-item active bg-info text-white" aria-current="page">
            <a class="link-white" href="admin/etiqueta/etiqueta.php">
              <?php echo $title_page; ?>
            </a>
          </li>
        </ol>
      ...
      <div class="container-full p-2 fs-x-14">
        <div class="row">
          ...
          <div class="col-12 mb-3">
            <a href="admin/etiqueta/etiqueta.php" class="btn btn-outline-primary btn-sm btn-bar" role="button">
              <i class="fas fa-list-ul"></i>
              Listar
            </a>
            <a href="admin/etiqueta/nuevo.php" class="btn btn-outline-primary btn-sm btn-bar" role="button">
            ...
            <table id="dataTableList" class="table table-sm table-hover table-bordered dt-responsive nowrap" style="width:100%">
              ...
              <tbody>
                ...
                <tr class="<?php echo $class_estado ;?>" >
                  ...
                  <td> <?php echo $row->etiqueta_id ?> </td>
                  <td> <?php echo $row->eti_descripcion ?> </td>

                  <td class="text-center">
                    <span class="sr-only"><?php echo $row->eti_publicar ?></span>
                    <button onclick="modalPublicar(<?php echo $row->etiqueta_id ?>, `<?php echo $row->eti_descripcion ?>` ,`<?php echo $title ?>`, `<?php echo $row->eti_publicar ?>`);" class="btn btn-sm lh-1 btn-table <?php echo $classBtn.' ' .$class_disabled; ; ?> " title="<?php echo $title; ?>" >
                    <?php echo $icon_pub ;?>
                    </button>
                  </td>
                  ...
                  <td class="text-center">
                    <a class="btn btn-outline-primary btn-sm lh-1 btn-table <?php echo $class_disabled ; ?>" href="admin/etiqueta/editar.php?id=<?php echo $row->etiqueta_id ?>" title="Editar">
                    <i class="fas fa-pencil-alt"></i>
                    </a>
                    <button class="btn btn-outline-danger btn-sm lh-1 btn-table" onclick="modalDelete(<?php echo $row->etiqueta_id ?>, `<?php echo $row->eti_descripcion ?>`,`<?php echo $title_estado ?>`,`<?php echo $row->eti_estado ?>`);" title="<?php echo $title_estado ;?>">
                    <i class="far fa-trash-alt"></i>
                    </button>
                    <span class="sr-only"><?php echo $row->eti_estado ?></span>
                  </td>
                </tr>
                <?php }?>
              </tbody>
  ...
  (function ($) {
    function processFormModal(event) {
      ...
      $.ajax({
        url: "./app/api/etiqueta/IndexEtiqueta.php",
        dataType: "json",
        type: "post",
