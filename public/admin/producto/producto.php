...
<body>
      ...
      <nav class="full-content" aria-label="breadcrumb">
        <ol class="breadcrumb breadcrumb-shape shadow-sm radius-0">
          ...
          <li class="breadcrumb-item active bg-info text-white" aria-current="page">
            <a class="link-white" href="admin/producto/producto.php">
              <?php echo $title_page; ?>
            </a>
          </li>
        </ol>
      ...
      <div class="container-full p-2 fs-x-14">
        <div class="row">
          ...
          <div class="col-12 mb-3">
            <a href="admin/producto/producto.php" class="btn btn-outline-primary btn-sm btn-bar" role="button">
              <i class="fas fa-list-ul"></i>
              Listar
            </a>
            <a href="admin/producto/nuevo.php" class="btn btn-outline-primary btn-sm btn-bar" role="button">
              ...
              <tbody>
                <?php foreach ($data as &$row) {?>
                  ...
                  <?php
                    ...
                    if(!empty($row->pro_publicar)){
                      if($row->pro_publicar == "S"){
                ...
                <tr class="<?php echo $class_estado ;?>" >
                  ...
                  <td class="text-center">
                    <span class="sr-only"><?php echo $row->pro_publicar ?></span>
                    <button onclick="modalPublicar(<?php echo $row->producto_id ?>, `<?php echo $row->sub_categoria_id ?>` ,`<?php echo $title ?>`, `<?php echo $row->pro_publicar ?>`);" class="btn btn-sm lh-1 btn-table <?php echo $classBtn.' ' .$class_disabled; ; ?> " title="<?php echo $title; ?>" >
                    <?php echo $icon_pub ;?>
                    </button>
                  </td>
                  ...
                  <td class="text-center">
                    <a class="btn btn-outline-primary btn-sm lh-1 btn-table <?php echo $class_disabled ; ?>" href="admin/producto/editar.php?id=<?php echo $row->producto_id ?>" title="Editar">
                    <i class="fas fa-pencil-alt"></i>
                    </a>
                    <button class="btn btn-outline-danger btn-sm lh-1 btn-table" onclick="modalDelete(<?php echo $row->producto_id ?>, `<?php echo $row->sub_categoria_id ?>`,`<?php echo $title_estado ?>`,`<?php echo $row->pro_estado ?>`);" title="<?php echo $title_estado ;?>">
                    <i class="far fa-trash-alt"></i>
                    </button>
                    <span class="sr-only"><?php echo $row->pro_estado ?></span>
                  </td>
                </tr>
                <?php }?>
              </tbody>
  ...
  (function ($) {
    function processFormModal(event) {
      ...
      $.ajax({
        url: "./app/api/producto/IndexProducto.php",
        dataType: "json",
        type: "post",
